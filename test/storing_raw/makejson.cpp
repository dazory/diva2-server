...
{
    string path = dir+"/JSON2/log.json";
...
{
    string path = dir+"/JSON2/sensor.json";
    Json::Value sensor;
    string sensor_name;
...
bool MakeJson::Frame()
{
    string path = dir+"/JSON2/frame.json";
    ifstream in(path.c_str());
...
bool MakeJson::Scene()
{
    string path = dir+"/JSON2/scene.json";
    ifstream in(path.c_str());
...
//     string path = dir+"/JSON2/lidar_data.json";
//     ifstream in(path.c_str());
...
//     for(int i=0; i<num_of_scene; i++){
//         while(1){
//             if(lidar_data_idx > lidar_data_start+199){
//                 lidar_data_start = lidar_data_idx;
//                 break;

//             }else if(lidar_data_idx > gps_last){
//                 break;
//             }
//             cam["token"] = Frame_data[where_frame_started]["frame_data_token"];
//             cam["sensor_token"] = Sensors[1]["sensor_token"];
//             cam["timestamp"]=iat->get_gps_timestamp(gps_data_idx);
//             this->Get_LLA(gps_data_idx);
//             cam["filename"] = lidar_path+ts_of_lidar_by_idx+".pcd";
//             cam["fileformat"] = "jpg";
...
//     string path = dir+"/JSON2/cam_data.json";
//     ifstream in(path.c_str());
//     if(in.is_open()) in >> Gps_data;
//     Json::Value cam;
...
//     for(int i=0; i<num_of_scene; i++){
//         while(1){
//             if(cam_data_idx > cam_data_start+199){
//                 cam_data_start = cam_data_idx;
//                 break;
...
//             cam["timestamp"]=iat->get_gps_timestamp(gps_data_idx);
//             this->Get_LLA(gps_data_idx);
//             cam["filename"] = cam_path+ts_of_cam_by_idx+".jpg";
//             cam["fileformat"] = "jpg";
...
{
    string path = dir+"/JSON2/gps_data.json";
    ifstream in(path.c_str());
    if(in.is_open()) in >> Gps_data;
    Json::Value gpses;
...
{
    string path = dir+"/JSON2/imu_data.json";
    ifstream in(path.c_str());
...
{
    string path = dir+"/JSON2/can_data.json";
    ifstream in(path.c_str());
